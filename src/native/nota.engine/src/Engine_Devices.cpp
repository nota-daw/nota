...
int32_t Engine::addTrackBuiltinDevice(int32_t trackId, int32_t kind) {
    ...
    else if (kind == 24) dev = std::make_shared<Phaser>();
    else if (kind == 25) dev = std::make_shared<Chorus>();
    else return -1;
...
std::shared_ptr<Device> Engine::cloneDevice(const Device& src) const {
    ...
    switch (src.builtinKind()) {                                               // built-ins
        ...
        case 24: d = std::make_shared<Phaser>(); break;
        case 25: d = std::make_shared<Chorus>(); break;
...
static std::shared_ptr<Device> makeBuiltinDevice(int32_t kind) {
    switch (kind) {
        ...
        case 24: return std::make_shared<Phaser>();
        case 25: return std::make_shared<Chorus>();

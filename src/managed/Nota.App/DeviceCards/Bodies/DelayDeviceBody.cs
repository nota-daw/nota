...
internal sealed class DelayDeviceBody : IDeviceBody
{
    ...
    public string? Subtitle => "DELAY";   // the processing type, shown as the header badge
    ...
    public Control Build(DeviceCardContext ctx, int index)
    {
        ...
        {
            var row = DeviceCardKit.SliderRow(name, () => P(p), n => { SetP(p, (float)n); SyncGraph(); }, () => fmt(P(p)), out var sync,
        ...
        {
            // Eight divisions: past the almanac's four-choice limit for segments — an accepted exception, see DESIGN.md § Accepted departures.
            var seg = DeviceCardKit.Segments(DivNames, () => Sel(p, 8), i => { SetP(p, i / 7f); SyncGraph(); }, out var sync);
            ctx.AddDeviceRefresher(sync);
        ...
        {
            int n = opts.Length;
            var seg = DeviceCardKit.Segments(opts, () => Sel(p, n), iv => { SetP(p, n > 1 ? iv / (float)(n - 1) : 0f); SyncGraph(); RebuildTime(); }, out var sync);
            ctx.AddDeviceRefresher(sync);
            MidiLearn.Bind(seg, MidiTarget.DeviceParam(track, di, p), engine.DeviceParamName(track, di, p));
            return seg;
        }
        ...
        Control Band(string title, Control body)
        {
            ...
            return new Border { Background = Card2, BorderBrush = BorderDef, BorderThickness = new Thickness(1), CornerRadius = NotaRadius.Panel, Padding = new Thickness(7, 5), Child = new StackPanel { Children = { head, body } } };
        }

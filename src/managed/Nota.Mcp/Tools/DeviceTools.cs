    ...
    : EngineTools(engine, dispatch, refresh)
{
    ...
    private static readonly (int Kind, string Name)[] Kinds =
    {
        ...
        (22, "Nota Lens"), (23, "Nota Flanger"), (24, "Nota Phaser"), (25, "Nota Chorus"),
    };
    ...
    public sealed record LevelReading(string Summary, string Live, string Mode, string State, double TargetLufs, string Reference,
        string Scale, string Response, double WindowSeconds, double GlideSeconds, double MaxGainDb, double TrimDb, double ManualGainDb,
        bool TruePeakSafe, double CeilingDbtp, double InputLufs, double OutputLufs, double InputMomentaryLufs, double OutputMomentaryLufs,
        double InputShortTermLufs, double OutputShortTermLufs, double TargetInEffectLufs, double DeltaLu, double CorrectionDb,
        double AppliedDb, double WantedDb, double LimiterDb, double TruePeakDbtp, double Correlation, string HeldBy, bool Primed,
        bool Silent, int LatencySamples, double SampleRate);

    private static readonly string[] LevelScales = { "Momentary", "ShortTerm", "Integrated" };

    [McpServerTool(Name = "read_level"), Description(
        "Read a Nota Level (built-in effect kind 18 — loudness leveler): the mode (Auto rides the gain to the target over the "
        + "glide; Manual holds ManualGainDb), its state (waiting for signal / silent, holding / on target within ±1 LU / tracking / "
        + "manual / held at ±max gain), the target (LUFS, or the reference track's loudness when one is routed as its sidechain), "
        + "the scale the correction reads (Momentary 400 ms / ShortTerm 3 s / Integrated 12 s with BS.1770 gating), Response, "
        + "Window and the glide it gives, Max Gain, Trim, True-peak safe with its ceiling, and the live numbers: input and output "
        + "loudness on the scale plus momentary and short-term, the distance to the target, the correction, the gain applied "
        + "(correction + trim − limiter), the limiter's reduction, true peak, correlation, what holds the gain (max gain, max cut, "
        + "true-peak), look-ahead latency. LUFS read −120 until measured; they move only while audio plays through the track.")]
    public Task<LevelReading> ReadLevel(int trackId, int deviceIndex) => Read(() => LevelRead(trackId, deviceIndex));

    private LevelReading LevelRead(int trackId, int deviceIndex)
    {
        if (E.TrackDeviceBuiltinKind(trackId, deviceIndex) != 18) throw new ArgumentException("not a Nota Level (kind 18)");
        var sc = new float[21];
        int n = E.DeviceScope(trackId, deviceIndex, sc, sc.Length);
        float V(int i) => n > i ? sc[i] : 0f;
        float L(int i) => n > i ? sc[i] : -120f;
        ...
        int pc = E.DeviceParamCount(trackId, deviceIndex);
        float Pm(int p) => p < pc ? E.DeviceGetParam(trackId, deviceIndex, p) : 0.5f;
        bool auto = Pm(2) >= 0.5f, fast = Pm(4) < 0.5f, primed = V(18) > 0.5f, silent = V(15) > 0.5f;
        int scale = Math.Clamp((int)Math.Round(Pm(1) * 2), 0, 2);
        double window = 0.4 * Math.Pow(25, Pm(5)), maxG = Pm(6) * 24;
        int clamp = (int)Math.Round(V(14));
        string held = V(20) > 0.3f && Pm(7) >= 0.5f ? "true-peak" : clamp == 1 ? "max gain" : clamp == 2 ? "max cut" : "";
        string state = !primed ? "waiting for signal" : silent ? "silent, holding" : !auto ? "manual"
            : clamp == 1 ? "held at +max gain" : clamp == 2 ? "held at -max gain" : Math.Abs(V(19)) < 1 ? "on target" : "tracking";
        int src = E.DeviceSidechainSource(trackId, deviceIndex);
        string reference = src >= 0 ? $"track {src}" + (E.GetTrackName(src) is { Length: > 0 } nm ? $" ({nm})" : "") : "none";
        return new LevelReading(E.DeviceText(trackId, deviceIndex, 0), E.DeviceText(trackId, deviceIndex, 1), auto ? "Auto" : "Manual", state,
            R1(-36 + Pm(0) * 36), reference, LevelScales[scale], fast ? "Fast" : "Slow", Math.Round(window, 2), Math.Round(window * (fast ? 0.25 : 1), 2),
            R1(maxG), R1((Pm(3) - 0.5) * 24), R1((Pm(9) - 0.5) * 48), Pm(7) >= 0.5f, R1(-6 + Pm(8) * 6),
            R1(L(9)), R1(L(10)), R1(L(2)), R1(L(11)), R1(L(0)), R1(L(1)), R1(L(3)), R1(V(19)), R1(V(12)), R1(V(4)), R1(V(8)), R1(V(13)),
            R1(L(5)), Math.Round(V(6), 2), held, primed, silent, (int)V(16), V(17));
    }

    [McpServerTool(Name = "set_level"), Description(
        "Play a Nota Level (built-in effect kind 18) in units — only the fields you pass change; each change is recorded like a hand "
        + "edit (so it lands in automation while recording). mode \"Auto\" | \"Manual\" (switching to Manual keeps the gain Auto was "
        + "riding); targetLufs −36..0 (−14 streaming, −16 podcast, −23 EBU R128, −24 ATSC); scale \"Momentary\" | \"ShortTerm\" | "
        + "\"Integrated\"; response \"Fast\" | \"Slow\" (Fast glides over a quarter of Window with a 5 ms look-ahead, Slow over all "
        + "of it with 20 ms); windowSeconds 0.4..10; maxGainDb 0..24; trimDb ±12; manualGainDb ±24 (limited to ±max gain; implies "
        + "nothing about mode); truePeakSafe; ceilingDbtp −6..0; referenceTrackId — a track whose loudness becomes the target "
        + "(−1 = none, the fixed target). Returns the device as read back.")]
    public Task<LevelReading> SetLevel(int trackId, int deviceIndex, string? mode = null, double? targetLufs = null, string? scale = null,
        string? response = null, double? windowSeconds = null, double? maxGainDb = null, double? trimDb = null, double? manualGainDb = null,
        bool? truePeakSafe = null, double? ceilingDbtp = null, int? referenceTrackId = null) => Mutate(() =>
        {
            if (E.TrackDeviceBuiltinKind(trackId, deviceIndex) != 18) throw new ArgumentException("not a Nota Level (kind 18)");
            void S(int p, double v) => ForgeSet(trackId, deviceIndex, p, v);
            if (targetLufs is { } t) S(0, (Math.Clamp(t, -36, 0) + 36) / 36);
            if (scale is not null)
            {
                int i = scale.Trim().ToLowerInvariant().Replace("-", "").Replace(" ", "") switch
                {
                    "momentary" or "mom" => 0, "shortterm" or "short" => 1, "integrated" or "integ" => 2,
                    _ => throw new ArgumentException("scale must be Momentary, ShortTerm or Integrated"),
                };
                S(1, i / 2.0);
            }
            if (response is not null)
            {
                bool f = response.Trim().ToLowerInvariant() switch
                {
                    "fast" => true, "slow" => false, _ => throw new ArgumentException("response must be Fast or Slow"),
                };
                S(4, f ? 0 : 1);
            }
            if (windowSeconds is { } w) S(5, Math.Log(Math.Clamp(w, 0.4, 10) / 0.4) / Math.Log(25));
            if (maxGainDb is { } mg) S(6, Math.Clamp(mg, 0, 24) / 24);
            if (trimDb is { } tr) S(3, 0.5 + Math.Clamp(tr, -12, 12) / 24);
            if (manualGainDb is { } g) S(9, 0.5 + Math.Clamp(g, -24, 24) / 48);
            if (truePeakSafe is { } sf) S(7, sf ? 1 : 0);
            if (ceilingDbtp is { } c) S(8, (Math.Clamp(c, -6, 0) + 6) / 6);
            if (mode is not null)
            {
                bool manual = mode.Trim().ToLowerInvariant() switch
                {
                    "manual" => true, "auto" => false, _ => throw new ArgumentException("mode must be Auto or Manual"),
                };
                bool wasAuto = E.DeviceGetParam(trackId, deviceIndex, 2) >= 0.5f;
                if (manual && wasAuto && manualGainDb is null)
                {
                    var sc = new float[21];
                    if (E.DeviceScope(trackId, deviceIndex, sc, sc.Length) > 12) S(9, 0.5 + Math.Round(sc[12], 1) / 48);
                }
                S(2, manual ? 0 : 1);
            }
            if (referenceTrackId is { } rt) E.SetDeviceSidechainSource(trackId, deviceIndex, rt < 0 ? -1 : rt);
            return LevelRead(trackId, deviceIndex);
        });

    [McpServerTool(Name = "level_match"), Description(
        "Nota Level (kind 18): switch to Manual with the gain set to the distance between the measured input and the target "
        + "(the card's MATCH) — a one-shot correction that then stays put. Needs audio to have been measured (read_level Primed). "
        + "Returns the device as read back.")]
    public Task<LevelReading> LevelMatch(int trackId, int deviceIndex) => Mutate(() =>
    {
        if (E.TrackDeviceBuiltinKind(trackId, deviceIndex) != 18) throw new ArgumentException("not a Nota Level (kind 18)");
        var sc = new float[21];
        int n = E.DeviceScope(trackId, deviceIndex, sc, sc.Length);
        if (n < 21 || sc[18] < 0.5f) throw new InvalidOperationException("nothing measured yet — play audio through the track first");
        double maxG = E.DeviceGetParam(trackId, deviceIndex, 6) * 24, trim = (E.DeviceGetParam(trackId, deviceIndex, 3) - 0.5) * 24;
        double want = Math.Clamp(sc[3] - sc[9] - trim, -maxG, maxG);
        ForgeSet(trackId, deviceIndex, 9, 0.5 + Math.Round(want, 1) / 48);
        ForgeSet(trackId, deviceIndex, 2, 0);
        return LevelRead(trackId, deviceIndex);
    });
    ...
    public sealed record FlangerReading(string Summary, string Live, string Waveform, bool Sync, string Division, double RateHz,
        double PeriodMs, double StereoDeg, double DelayMs, double DepthPercent, double FeedbackPercent, double MixPercent, string Mode,
        double TauMinMs, double TauMaxMs, double TauLMs, double TauRMs, double NotchHz, double NotchMinHz, double NotchMaxHz,
        double NullDb, double PeakDb, double WindowPhase, bool Locked, bool Playing, double Bpm, double InputPeakDb,
        double OutputPeakLDb, double OutputPeakRDb, bool Signal, double SampleRate);

    private static readonly string[] FlangerWaves = { "Sine", "Triangle", "Saw" };
    private static readonly string[] FlangerDivs = { "2/1", "1/1", "1/2", "1/4D", "1/4", "1/4T", "1/8", "1/8T", "1/16" };

    [McpServerTool(Name = "read_flanger"), Description(
        "Read what a Nota Flanger (built-in effect kind 23 — a comb filter on a short LFO-swept delay) is doing: the LFO "
        + "waveform, free rate (Hz) or Sync division, the period, Stereo (the right LFO's offset, 0..180°), Delay (the centre of "
        + "the sweep, ms), Depth, Feedback (−95..+95 %) and the mode it makes (positive = jet, negative = hollow, resonant past "
        + "85 %, through near 0), Mix; the sweep's delay range (ms), live τ per channel, the first notch now and the range it "
        + "sweeps (Hz), the notch depth and the comb's peak level (dB), the LFO's place in its two-cycle window, whether it is "
        + "locked to the song (Sync + playing), tempo and input / L / R output peaks (dBFS, −120 = silence). Live values move "
        + "only while audio runs through the track. Params: 0 Rate, 1 Delay, 2 Depth, 3 Feedback, 4 Mix, 5 Waveform, 6 Sync, "
        + "7 Division, 8 Stereo — set them in units with set_flanger; device_action 0 restarts the LFO, 1 clears the delay "
        + "line, 2 resets the meters.")]
    public Task<FlangerReading> ReadFlanger(int trackId, int deviceIndex) => Read(() => FlangerRead(trackId, deviceIndex));

    private FlangerReading FlangerRead(int trackId, int deviceIndex)
    {
        if (E.TrackDeviceBuiltinKind(trackId, deviceIndex) != 23) throw new ArgumentException("not a Nota Flanger (kind 23)");
        var sc = new float[32];
        int n = E.DeviceScope(trackId, deviceIndex, sc, sc.Length);
        float V(int i) => n > i ? sc[i] : 0f;
        float Db(int i) => n > i ? sc[i] : -120f;
        ...
        float Pm(int p) => E.DeviceGetParam(trackId, deviceIndex, p);
        int wave = Math.Clamp((int)Math.Round(Pm(5) * 2), 0, 2);
        int div = Math.Clamp((int)Math.Round(Pm(7) * 8), 0, 8);
        double fb = (Pm(3) - 0.5) * 1.9;
        string mode = Math.Abs(fb) >= 0.85 ? "resonant" : fb <= -0.15 ? "negative" : fb >= 0.15 ? "positive" : "through";
        double rate = V(6);
        return new FlangerReading(E.DeviceText(trackId, deviceIndex, 0), E.DeviceText(trackId, deviceIndex, 1),
            FlangerWaves[wave], Pm(6) >= 0.5f, FlangerDivs[div], Math.Round(rate, 3), rate > 0 ? Math.Round(1000 / rate) : 0,
            Math.Round(Pm(8) * 180), R2(0.1 * Math.Pow(80, Pm(1))), R1(Pm(2) * 100), R1(fb * 100), R1(Pm(4) * 100), mode,
            R2(V(12)), R2(V(13)), R2(V(3)), R2(V(4)), Math.Round(V(14)), Math.Round(V(15)), Math.Round(V(16)),
            R1(V(17)), R1(V(18)), Math.Round(V(5), 3), V(9) > 0.5f, V(8) > 0.5f, Math.Round(V(7), 2),
            R1(Db(0)), R1(Db(1)), R1(Db(2)), V(22) > 0.5f, V(10));
    }

    [McpServerTool(Name = "set_flanger"), Description(
        "Play a Nota Flanger (built-in effect kind 23) in units — only the fields you pass change; each change is recorded like "
        + "a hand edit (so it lands in automation while recording). waveform \"Sine\" | \"Triangle\" | \"Saw\"; rateHz 0.02..8 "
        + "(sets free run); division \"2/1\" \"1/1\" \"1/2\" \"1/4D\" \"1/4\" \"1/4T\" \"1/8\" \"1/8T\" \"1/16\" (sets Sync); "
        + "sync true / false; stereoDeg 0..180 (0 = both channels sweep together, 180 = opposite); delayMs 0.1..8 (the centre "
        + "of the sweep — shorter = higher notches); depthPercent 0..100; feedbackPercent −95..+95 (positive = jet, negative = "
        + "hollow, past ±85 the comb rings); mixPercent 0..100 (50 = the deepest notches, 100 = wet only, a vibrato). Returns "
        + "the device as read back.")]
    public Task<FlangerReading> SetFlanger(int trackId, int deviceIndex, string? waveform = null, double? rateHz = null,
        string? division = null, bool? sync = null, double? stereoDeg = null, double? delayMs = null, double? depthPercent = null,
        double? feedbackPercent = null, double? mixPercent = null) => Mutate(() =>
        {
            if (E.TrackDeviceBuiltinKind(trackId, deviceIndex) != 23) throw new ArgumentException("not a Nota Flanger (kind 23)");
            void S(int p, double v) => ForgeSet(trackId, deviceIndex, p, v);
            if (waveform is not null)
            {
                int w = waveform.Trim().ToLowerInvariant() switch
                {
                    "sine" or "sin" => 0, "triangle" or "tri" => 1, "saw" or "sawtooth" => 2,
                    _ => throw new ArgumentException("waveform must be Sine, Triangle or Saw"),
                };
                S(5, w / 2.0);
            }
            if (rateHz is { } hz) { S(6, 0); S(0, Math.Log(Math.Clamp(hz, 0.02, 8) / 0.02) / Math.Log(400)); }
            if (sync is { } sy) S(6, sy ? 1 : 0);
            if (division is not null)
            {
                int d = Array.IndexOf(FlangerDivs, division.Trim().ToUpperInvariant());
                if (d < 0) throw new ArgumentException("division must be one of " + string.Join(", ", FlangerDivs));
                S(6, 1); S(7, d / 8.0);
            }
            if (stereoDeg is { } st) S(8, Math.Clamp(st, 0, 180) / 180);
            if (delayMs is { } dm) S(1, Math.Log(Math.Clamp(dm, 0.1, 8) / 0.1) / Math.Log(80));
            if (depthPercent is { } dp) S(2, dp / 100);
            if (feedbackPercent is { } fp) S(3, 0.5 + Math.Clamp(fp, -95, 95) / 190);
            if (mixPercent is { } mp) S(4, mp / 100);
            return FlangerRead(trackId, deviceIndex);
        });

    public sealed record PhaserReading(string Summary, string Live, int Stages, int Notches, string Waveform, bool Sync, string Division,
        double RateHz, double PeriodMs, double StereoDeg, double CenterHz, double DepthOctaves, double FeedbackPercent, double MixPercent,
        string Mode, double FcMinHz, double FcMaxHz, double FcLHz, double FcRHz, double NotchHz, double NotchMinHz, double NotchMaxHz,
        double NullDb, double PeakDb, double WindowPhase, bool Locked, bool Playing, double Bpm, double InputPeakDb,
        double OutputPeakLDb, double OutputPeakRDb, bool Signal, double SampleRate);

    private static readonly string[] PhaserWaves = { "Sine", "Triangle", "Saw" };
    private static readonly string[] PhaserDivs = { "4/1", "2/1", "1/1", "1/2", "1/4D", "1/4", "1/4T", "1/8", "1/16" };
    private static readonly int[] PhaserStages = { 2, 4, 6, 8, 12 };

    [McpServerTool(Name = "read_phaser"), Description(
        "Read what a Nota Phaser (built-in effect kind 24 — a chain of 2 / 4 / 6 / 8 / 12 all-pass stages an LFO sweeps; "
        + "notches = stages / 2, one fewer with negative feedback) is doing: the stages and notches, the LFO waveform, free rate "
        + "(Hz) or Sync division, the period, Stereo (the right LFO's offset, 0..180°), Center (the middle of the sweep, Hz), "
        + "Depth (± octaves), Feedback (−95..+95 %) and the mode it makes (positive, negative, resonant past 85 %, clean near 0), "
        + "Mix; the corner's sweep range and live fc per channel (Hz), the first notch now and the range it sweeps, the notch "
        + "depth and the peak level (dB), the LFO's place in its two-cycle window, whether it is locked to the song (Sync + "
        + "playing), tempo and input / L / R output peaks (dBFS, −120 = silence). Live values move only while audio runs through "
        + "the track. Params: 0 Rate, 1 Center, 2 Depth, 3 Feedback, 4 Mix, 5 Waveform, 6 Sync, 7 Division, 8 Stereo, 9 Stages — "
        + "set them in units with set_phaser; device_action 0 restarts the LFO, 1 clears the stages, 2 resets the meters.")]
    public Task<PhaserReading> ReadPhaser(int trackId, int deviceIndex) => Read(() => PhaserRead(trackId, deviceIndex));

    private PhaserReading PhaserRead(int trackId, int deviceIndex)
    {
        if (E.TrackDeviceBuiltinKind(trackId, deviceIndex) != 24) throw new ArgumentException("not a Nota Phaser (kind 24)");
        var sc = new float[32];
        int n = E.DeviceScope(trackId, deviceIndex, sc, sc.Length);
        float V(int i) => n > i ? sc[i] : 0f;
        float Db(int i) => n > i ? sc[i] : -120f;
        ...
        int wave = Math.Clamp((int)Math.Round(Pm(5) * 2), 0, 2);
        int div = Math.Clamp((int)Math.Round(Pm(7) * 8), 0, 8);
        int stages = PhaserStages[Math.Clamp((int)Math.Round(Pm(9) * 4), 0, 4)];
        double fb = (Pm(3) - 0.5) * 1.9;
        string mode = Math.Abs(fb) >= 0.85 ? "resonant" : fb <= -0.15 ? "negative" : fb >= 0.15 ? "positive" : "clean";
        double rate = V(6);
        return new PhaserReading(E.DeviceText(trackId, deviceIndex, 0), E.DeviceText(trackId, deviceIndex, 1),
            stages, fb < 0 ? stages / 2 - 1 : stages / 2, PhaserWaves[wave], Pm(6) >= 0.5f, PhaserDivs[div],
            Math.Round(rate, 3), rate > 0 ? Math.Round(1000 / rate) : 0, Math.Round(Pm(8) * 180),
            Math.Round(50 * Math.Pow(100, Pm(1))), R2(Pm(2) * 3), R1(fb * 100), R1(Pm(4) * 100), mode,
            Math.Round(V(12)), Math.Round(V(13)), Math.Round(V(3)), Math.Round(V(4)), Math.Round(V(14)), Math.Round(V(15)), Math.Round(V(16)),
            R1(V(17)), R1(V(18)), Math.Round(V(5), 3), V(9) > 0.5f, V(8) > 0.5f, Math.Round(V(7), 2),
            R1(Db(0)), R1(Db(1)), R1(Db(2)), V(22) > 0.5f, V(10));
    }

    [McpServerTool(Name = "set_phaser"), Description(
        "Play a Nota Phaser (built-in effect kind 24) in units — only the fields you pass change; each change is recorded like a "
        + "hand edit (so it lands in automation while recording). stages 2 | 4 | 6 | 8 | 12 (notches = stages / 2); waveform "
        + "\"Sine\" | \"Triangle\" | \"Saw\"; rateHz 0.02..8 (sets free run); division \"4/1\" \"2/1\" \"1/1\" \"1/2\" \"1/4D\" "
        + "\"1/4\" \"1/4T\" \"1/8\" \"1/16\" (sets Sync); sync true / false; stereoDeg 0..180 (0 = both channels sweep together, "
        + "180 = opposite); centerHz 50..5000 (the middle of the sweep); depthOctaves 0..3 (± around Center); feedbackPercent "
        + "−95..+95 (positive sharpens the peaks, negative moves and widens the notches, past ±85 it whistles); mixPercent 0..100 "
        + "(50 = the deepest notches). Returns the device as read back.")]
    public Task<PhaserReading> SetPhaser(int trackId, int deviceIndex, int? stages = null, string? waveform = null, double? rateHz = null,
        string? division = null, bool? sync = null, double? stereoDeg = null, double? centerHz = null, double? depthOctaves = null,
        double? feedbackPercent = null, double? mixPercent = null) => Mutate(() =>
        {
            if (E.TrackDeviceBuiltinKind(trackId, deviceIndex) != 24) throw new ArgumentException("not a Nota Phaser (kind 24)");
            void S(int p, double v) => ForgeSet(trackId, deviceIndex, p, v);
            if (stages is { } st)
            {
                int i = Array.IndexOf(PhaserStages, st);
                if (i < 0) throw new ArgumentException("stages must be 2, 4, 6, 8 or 12");
                S(9, i / 4.0);
            }
            if (waveform is not null)
            {
            ...
            if (division is not null)
            {
                int d = Array.IndexOf(PhaserDivs, division.Trim().ToUpperInvariant());
                if (d < 0) throw new ArgumentException("division must be one of " + string.Join(", ", PhaserDivs));
                S(6, 1); S(7, d / 8.0);
            }
            if (stereoDeg is { } sd) S(8, Math.Clamp(sd, 0, 180) / 180);
            if (centerHz is { } ch) S(1, Math.Log(Math.Clamp(ch, 50, 5000) / 50) / Math.Log(100));
            if (depthOctaves is { } dp) S(2, Math.Clamp(dp, 0, 3) / 3);
            if (feedbackPercent is { } fp) S(3, 0.5 + Math.Clamp(fp, -95, 95) / 190);
            if (mixPercent is { } mp) S(4, mp / 100);
            return PhaserRead(trackId, deviceIndex);
        });

    public sealed record ChorusVoiceReading(string Name, double DelayMs, double Cents);
    public sealed record ChorusReading(string Summary, string Live, string Mode, int Voices, bool Sync, string Division, double RateHz,
        double PeriodMs, double OffsetDeg, double HpfHz, double AmountPercent, double FeedbackPercent, double WidthPercent,
        double WarmthPercent, double MixPercent, string State, double DelayMinMs, double DelayMaxMs, double MaxCents,
        IReadOnlyList<ChorusVoiceReading> VoicesNow, double WindowPhase, bool Locked, bool Playing, double Bpm, double InputPeakDb,
        double OutputPeakLDb, double OutputPeakRDb, bool Signal, double SampleRate);

    private static readonly string[] ChorusModes = { "Classic", "Ensemble", "Vibrato" };
    private static readonly string[] ChorusDivs = { "4/1", "2/1", "1/1", "1/2", "1/4", "1/4T", "1/8", "1/8T", "1/16" };

    [McpServerTool(Name = "read_chorus"), Description(
        "Read what a Nota Chorus (built-in effect kind 25 — modulated delay voices: Classic = 2 voices L / R on 8 ± 5 ms, Ensemble = "
        + "3 voices 120° apart on 12 ± 6 ms, Vibrato = the L / R pair on 3 ± 2.5 ms, wet only) is doing: the mode and voice count, "
        + "the sine LFO's free rate (Hz) or Sync division and period, Offset (the right voice's LFO behind the left, 0..180°; "
        + "fixed in Ensemble), the wet HPF (Hz, 0 = off), Amount (share of the mode's swing), Feedback (−90..+90 %), Width "
        + "(0..200 %), Warmth, Mix (Vibrato: always 100), the state (2 voices, 3 voices, wet only, resonant past ±60 % feedback, "
        + "bypass); the delay range each voice sweeps, the largest detune (± cents), each voice's delay and detune now, the LFO's "
        + "place in its two-cycle window, whether it is locked to the song (Sync + playing), tempo and input / L / R output peaks "
        + "(dBFS, −120 = silence). Live values move only while audio runs through the track. Params: 0 Rate, 1 Mode, 2 Sync, "
        + "3 Division, 4 Offset, 5 HPF, 6 Amount, 7 Feedback, 8 Width, 9 Warmth, 10 Mix — set them in units with set_chorus; "
        + "device_action 0 restarts the LFO, 1 clears the delay lines, 2 resets the meters.")]
    public Task<ChorusReading> ReadChorus(int trackId, int deviceIndex) => Read(() => ChorusRead(trackId, deviceIndex));

    private ChorusReading ChorusRead(int trackId, int deviceIndex)
    {
        if (E.TrackDeviceBuiltinKind(trackId, deviceIndex) != 25) throw new ArgumentException("not a Nota Chorus (kind 25)");
        var sc = new float[32];
        int n = E.DeviceScope(trackId, deviceIndex, sc, sc.Length);
        float V(int i) => n > i ? sc[i] : 0f;
        float Db(int i) => n > i ? sc[i] : -120f;
        ...
        static double R2(double v) => Math.Round(v, 2);
        float Pm(int p) => E.DeviceGetParam(trackId, deviceIndex, p);
        int mode = Math.Clamp((int)Math.Round(Pm(1) * 2), 0, 2);
        int div = Math.Clamp((int)Math.Round(Pm(3) * 8), 0, 8);
        int voices = mode == 1 ? 3 : 2;
        double fb = (Pm(7) - 0.5) * 1.8;
        bool bypassed = E.DeviceBypassed(trackId, deviceIndex);
        string state = bypassed ? "bypass" : mode == 2 ? "wet only" : Math.Abs(fb) >= 0.6 ? "resonant" : voices == 3 ? "3 voices" : "2 voices";
        double rate = V(10);
        double hp = Pm(5) < 0.01f ? 0 : 20 * Math.Pow(100, Pm(5));
        string[] names = mode == 1 ? new[] { "V1", "V2", "V3" } : new[] { "L", "R" };
        var now = names.Select((nm, i) => new ChorusVoiceReading(nm, R2(V(3 + i)), R1(V(6 + i)))).ToList();
        return new ChorusReading(E.DeviceText(trackId, deviceIndex, 0), E.DeviceText(trackId, deviceIndex, 1),
            ChorusModes[mode], voices, Pm(2) >= 0.5f, ChorusDivs[div], Math.Round(rate, 3), rate > 0 ? Math.Round(1000 / rate) : 0,
            mode == 1 ? 120 : Math.Round(Pm(4) * 180), Math.Round(hp), R1(Pm(6) * 100), R1(fb * 100), Math.Round(Pm(8) * 200),
            R1(Pm(9) * 100), mode == 2 ? 100 : R1(Pm(10) * 100), state, R2(V(16)), R2(V(17)), R1(V(18)), now,
            Math.Round(V(9), 3), V(13) > 0.5f, V(12) > 0.5f, Math.Round(V(11), 2), R1(Db(0)), R1(Db(1)), R1(Db(2)), V(26) > 0.5f, V(14));
    }

    [McpServerTool(Name = "set_chorus"), Description(
        "Play a Nota Chorus (built-in effect kind 25) in units — only the fields you pass change; each change is recorded like a "
        + "hand edit (so it lands in automation while recording). mode \"Classic\" | \"Ensemble\" | \"Vibrato\" (Vibrato is wet "
        + "only); rateHz 0.02..8 (sets free run); division \"4/1\" \"2/1\" \"1/1\" \"1/2\" \"1/4\" \"1/4T\" \"1/8\" \"1/8T\" "
        + "\"1/16\" (sets Sync); sync true / false; offsetDeg 0..180 (Classic / Vibrato: 0 = both voices move together, 180 = "
        + "opposite); hpfHz 0 = off or 20..2000 (keeps the lows out of the wet); amountPercent 0..100 (how far the delay swings); "
        + "feedbackPercent −90..+90 (past ±60 it rings metallic); widthPercent 0..200 (0 = mono, 100 = as is, 200 = side +6 dB); "
        + "warmthPercent 0..100 (a dark, saturated bucket-brigade line); mixPercent 0..100 (50 = dry and wet equal). Returns the "
        + "device as read back.")]
    public Task<ChorusReading> SetChorus(int trackId, int deviceIndex, string? mode = null, double? rateHz = null, string? division = null,
        bool? sync = null, double? offsetDeg = null, double? hpfHz = null, double? amountPercent = null, double? feedbackPercent = null,
        double? widthPercent = null, double? warmthPercent = null, double? mixPercent = null) => Mutate(() =>
        {
            if (E.TrackDeviceBuiltinKind(trackId, deviceIndex) != 25) throw new ArgumentException("not a Nota Chorus (kind 25)");
            void S(int p, double v) => ForgeSet(trackId, deviceIndex, p, v);
            if (mode is not null)
            {
                int m = mode.Trim().ToLowerInvariant() switch
                {
                    "classic" => 0, "ensemble" => 1, "vibrato" => 2,
                    _ => throw new ArgumentException("mode must be Classic, Ensemble or Vibrato"),
                };
                S(1, m / 2.0);
            }
            if (rateHz is { } hz) { S(2, 0); S(0, Math.Log(Math.Clamp(hz, 0.02, 8) / 0.02) / Math.Log(400)); }
            if (sync is { } sy) S(2, sy ? 1 : 0);
            if (division is not null)
            {
                int d = Array.IndexOf(ChorusDivs, division.Trim().ToUpperInvariant());
                if (d < 0) throw new ArgumentException("division must be one of " + string.Join(", ", ChorusDivs));
                S(2, 1); S(3, d / 8.0);
            }
            if (offsetDeg is { } od) S(4, Math.Clamp(od, 0, 180) / 180);
            if (hpfHz is { } hp) S(5, hp < 20 ? 0 : Math.Log(Math.Clamp(hp, 20, 2000) / 20) / Math.Log(100));
            if (amountPercent is { } ap) S(6, Math.Clamp(ap, 0, 100) / 100);
            if (feedbackPercent is { } fp) S(7, 0.5 + Math.Clamp(fp, -90, 90) / 180);
            if (widthPercent is { } wp) S(8, Math.Clamp(wp, 0, 200) / 200);
            if (warmthPercent is { } wm) S(9, Math.Clamp(wm, 0, 100) / 100);
            if (mixPercent is { } mp) S(10, Math.Clamp(mp, 0, 100) / 100);
            return ChorusRead(trackId, deviceIndex);
        });
